...
async def add_video(interaction:discord.Interaction, url:str=None, filename:str="Change Me!"):
    ...
    try:
        ...
        if place >= 0:
            await interaction.response.edit_message(content=f"Video downloaded! It is number {place+1} in the queue!")
        else:
            await interaction.response.edit_message(content=f"Video downloaded! It's loaded and ready to play!")
    except:
        await interaction.response.edit_message(content=f"Something went wrong with the video download, are you sure you gave it a unique filename?")
